...
figure
plot(year_vec,informal_size,'-o',year_vec,informal_size_model_reported,'linewidth',2)
...
figure
plot(1:10,h_evasion(:,1)./(tfp_vec(1)*lambda_vec),'-o',...
    1:10,h_evasion(:,7)./(tfp_vec(7)*lambda_vec),...
    1:10,h_evasion(:,9)./(tfp_vec(9)*lambda_vec),...
    1:10,h_evasion(:,15)./(tfp_vec(15)*lambda_vec),'-+','linewidth',2)
axis([-inf inf 0 1])
...
% fprintf(fid,'\n  Gini &  $%5.3f$ & $%5.3f$  \\\\ ',0.332, Gini_y_evasion);
